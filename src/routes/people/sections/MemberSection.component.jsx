...
const PeopleSection = () => {
  ...
  return (
    <div className="p-4 my-32 max-w-screen-xl mx-auto">
      <p className="text-6xl font-raleway text-center mb-4">Our Lab Members</p>
      <p className="text-xl font-lato font-light text-center mb-8 mx-auto md:w-2/5">
      ...
      <div className="flex shadow p-2 rounded-xl w-1/3">
        ...
        />
        <input
          className="text-xl ml-2 outline-none text-gray-500"
      ...
      <div className="flex flex-wrap justify-evenly">
        {loading ? (
          ...
          people
            ?.filter(
              (member) =>
                member.data.M.first.S.toLowerCase().includes(
                  search.toLocaleLowerCase(),
                ) ||
                member.data.M.last.S.toLowerCase().includes(
                  search.toLocaleLowerCase(),
                ),
            )
            .map((member) => {
              return <Profile key={member.data.M} data={member.data.M} />;
            })
        )}
      </div>
    </div>
