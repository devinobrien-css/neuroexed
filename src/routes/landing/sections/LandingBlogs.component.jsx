...
const LandingBlogs = () => {
    ...
    return (
        ...
        >
            <p className='text-6xl font-raleway font-light mb-8'>Blog Posts and Podcasts</p>
                {blogs?(
                    <>
                        <div className='flex gap-x-6'>
                            <div className='md:w-1/2 space-y-12 h-max-content flex flex-col justify-between'>
                                <BlogMd data={blogs[paginate].data.M} />
                                <BlogStyledMd data={blogs[paginate+1].data.M} index={paginate+1}/>
                            </div>
                            <div className='md:w-1/2 space-y-12 h-max-content flex flex-col justify-between'>

                                <BlogStyledMd data={blogs[paginate+2].data.M} index={paginate+2}/>
                                <BlogMd data={blogs[paginate+3].data.M} />
                            </div>
                        </div>
                        <div className='w-full shadow flex justify-between'>
                            <button 
                                className={`py-6 border-l px-8 group ${paginate-step>=0?"":"cursor-not-allowed"}`}
                            ...
                            }><Icon icon="lucide:chevron-first" className='text-4xl text-gray-300 rounded-full group-hover:bg-gray-200 group-hover:shadow-xl group-hover:text-white transition-colors duration-500' /></button>
                            <p className='italic text-gray-400 font-raleway text-lg my-6'>page {page} of {Math.round(blogs?.length/step)}</p>
                            <button 
                                className={`group py-6 border-r px-8 ${(paginate+step>=blogs?.length-1)?"cursor-not-allowed":""}`}
                            ...
                            }><Icon icon="lucide:chevron-last" className='text-4xl text-gray-300 rounded-full group-hover:bg-gray-200 group-hover:shadow-xl group-hover:text-white transition-colors duration-500' /></button>
                        </div>
                    </>
                ):(<Loader />)}
