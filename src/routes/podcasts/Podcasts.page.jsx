...
import { SectionTitle, TitleXl } from "../../shared/components/common.library";
        ...
        transcript ? (
          <div className="absolute w-full h-full flex flex-col top-0 left-0 bg-gray-800 bg-opacity-30 z-[10001]">
            <div className="w-3/5 p-8 my-auto mx-auto bg-white rounded relative space-y-4">
              <p className="text-gray-600 font-light text-xl mx-auto block text-center">
                {podcast.season.S}.{podcast.episode.S}{" "}
                <span className="md:text-4xl text-lg font-bold font-lato text-black">
                  {podcast.title.S}
                </span>
              </p>
              ...
              <audio controls id="download-me" className="mx-auto">
                <source
                  src={`https://neuroexed-bucket.s3.us-east-1.amazonaws.com/podcasts/${podcast.season.S}.${podcast.episode.S}`}
                  type="audio/mpeg"
                />
              </audio>
              <button
                className="absolute top-4 left-4 bg-gray-300 rounded-lg p-2"
          ...
          <div className="absolute w-full h-full flex flex-col top-0 left-0 bg-gray-800 bg-opacity-30 z-[10001]">
            <div className="w-3/5 p-8 my-auto mx-auto bg-white rounded relative">
              <button
                className="absolute top-4 right-4 bg-gray-300 rounded-lg p-2"
                onClick={() => setModal()}
              >
                close
              </button>
              ...
              <p className="text-gray-600 font-light">
                {podcast.season.S}.{podcast.episode.S}{" "}
                <span className="text-xl font-bold font-lato text-black">
                  {podcast.title.S}
                </span>
              </p>
              ...
              <div className="md:flex gap-8">
                <div className="md:w-1/2">
                  ...
                  <div>
                    {podcast.contributors?.L.map((person, index) => {
                      return (
                        <p key={index} className="text-gray-800 font-light">
                          {person.S}
                        </p>
                      );
                    })}
                ...
                <div>
                  ...
                  <br />
                  <button
                    className="font-lato rounded-xl p-3 bg-gray-100 text-center w-full"
          ...
          <div className="p-2">
            <div className="flex">
              <div className="w-full">
                <p className="text-gray-600 font-light text-4xl">
                  {podcast.season.S}.{podcast.episode.S}{" "}
                  <span className="text-xl font-normal font-lato text-black">
                    {podcast.title.S}
                  </span>
                </p>
                <p className=" text-gray-500 font-sans font-light text-lg italic">
                  Released on {new Date(podcast.date.S).toLocaleDateString()}
                </p>
              </div>
              <div className="flex justify-between w-1/4  my-auto h-full">
                <div
                  className="relative rounded-full p-2 hover:shadow hover:bg-gray-200 cursor-pointer active:shadow-xl group"
                  onClick={() => setModal(true)}
                >
                  <Icon icon="bx:headphone" width="25" className="" />
                  <p className="z-50 group-hover:opacity-100 group-hover:w-12 group-hover:p-2 overflow-hidden opacity-0 w-0 transition-all absolute bottom-7 right-full bg-gray-200 p-0 rounded">
                    listen
                  </p>
                </div>
                ...
                >
                  ...
                  <p className="group-hover:opacity-100 group-hover:w-24 text-center group-hover:p-2 overflow-hidden opacity-0 w-0 transition-all absolute bottom-7 right-full bg-gray-200 p-0 rounded">
                    download
                  </p>
                </div>
              </div>
  ...
  return (
    <div className="">
      <NavHeader
        ...
        content="Thia podcast explores the process of learning from a direct experience in all of its forms."
      />

      <div className="max-w-screen-xl mx-auto my-16">
        <SectionTitle className="w-fit mx-auto">Episodes</SectionTitle>
        <br />
        ...
        {podcasts
          ?.filter((podcast) => Number(podcast.season.S) === 1)
          .sort(
            (a, b) => Number(b.data.M.episode.S) - Number(a.data.M.episode.S),
          )
          .map((podcast) => {
            console.log(podcast);
            return <Podcast podcast={podcast.data.M} />;
          })}
      </div>
    </div>
  );
