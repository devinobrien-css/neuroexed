    ...
    <div className="editable selected-editable" id={'new-post'}>
      ...
      <div className={state ? 'hidden-content open' : 'hidden-content'}>
        ...
        <div className={'border-2 px-2'}>
          ...
          <input
            className="my-auto block h-min w-min cursor-pointer overflow-hidden rounded-lg border border-gray-300 bg-gray-50 text-sm text-gray-900 focus:outline-none "
            type="file"
            accept=".mp3,audio/*"
            onChange={(event) => {
        ...
        <div className={'border-2 p-2'}>
          <div className="flex justify-between">
            ...
            <button className="browser-btn" onClick={handleAddString}>
              Add Contributor
            </button>
          </div>
          <div>
            {contributors.map((contributor, index) => {
              return (
                <div className={`justify-between border-t md:flex`}>
                  <label className="w-3/12 text-gray-500">Name</label>
                  ...
                  <input
                    ...
                    className="w-full border-0 md:border-l md:border-gray-300 md:pl-2"
                  />
                  <button className="browser-btn" onClick={handleRemoveString}>
                    X
                  </button>
                </div>
              );
...
const EditablePodcast = (args) => {
  ...
  const [imageUpload, setImageUpload] = useState('');
  const [contributors, setContributors] = useState(
    args.data.contributors?.L.map((item) => item.S) ?? [],
  );
    ...
    return (
      <div className="editable-list">
        <div>
          ...
          />
          <button className="browser-btn" onClick={() => setNewPodcast(true)}>
            add new podcast
          </button>
        </div>
